---
title: "Dable의 i18n 시스템"
date: 2021-02-24
description: "Dable의 i18n system 구조에 대한 글"
tags:
  - i18n
  - js
banner: ./dable-i18n-system.png
---

이 글은 제가 재직중인 Dable의 techblog에 작성한 i18n(국제화) system에 대한 글입니다.
개발 블로그의 첫 글로 올리기에 좋은 글인지는 잘 모르겠습니다.
사내 시스템을 코드 레벨까지 내려가서 공개하는 것은 좋지 않은 것 같아서 시스템을 말로 표현하다보니 애매해진 부분도 있는 것 같습니다.
언젠가 시스템 개선 작업을 진행하게 되면 바뀐 부분을 다시 올려보려고 합니다.
...
![데이블의 i18n 시스템 구조를 그린 도표. i18n dashboard라는 서비스에 데이터베이스, AWS S3 저장소, Redis Elasticache 등의 서비스가 연결되어서 번역키의 배포를 위해 사용되고, 각 서비스 repository는 배포 과정에서 i18n-dashboard와 상호 작용하며 번역키의 수정, 추가, 삭제를 적용하고 동기화한다.](./dable-i18n-system.png)
...
#### Pull Request
![데이블의 i18n 시스템이 동작하는 프로세스를 나타낸 그림. github에 Pull Request를 올렸을 때 어떤 일이 일어나는지 그리고 있다. PR이 올라온 브랜치에 번역키가 추가된 내용이 있으면 DB에 추가하고 영어 번역 상태를 체크한다. 영어 번역이 완료된 경우 PR을 승인한다.](./i18n-process-pr.png)
...
![i18n 시스템의 merge 과정을 나타낸 그림. 풀 리퀘스트가 merge되면 번역키의 브랜치 정보도 모두 master로 변경하고 삭제된 키를 지운 후에, 현재의 번역키 전부를 json 형태로 s3에 저장한다.](./i18n-process-merge.png)
...
### Live
![live server에서 i18n system이 동작하는 구조를 나타낸 그림. i18n-dashboard는 10분마다 Redis에 번역 키 전체를 밀어넣고, 각 서버는 1분마다 레디스를 확인하여 번역키에 수정이 생겼으면 키를 받아와서 수정함. 그리고 i18n-dashboard는 매 월요일마다 번역이 완료되지 않은 서비스의 번역 현황을 퍼센트로 계산하여 슬랙에 전송한다.](./i18n-process-live.png)
